...
import type { Dish, EquivalentType } from "@/types";
...
export const initDB = async () => {
  const db = await openDB<FavoritesDB>(DB_NAME, DB_VERSION, {
    ...
    blocked() {
      console.warn("Una versión anterior está bloqueando la actualización de la base de datos");
    },
    blocking() {
...
export async function addFavorite(period: string, dish: Dish, equivalent: Array<EquivalentType>) {
  try {
